...
const config = {
  libraryName: 'ngx-bootstrap-notify',
  unscopedLibraryName: 'ngx-bootstrap-notify',
  allSrc: 'src/**/*',
  allTs: 'src/**/!(*.spec).ts',
...
gulp.task('rollup-bundle', (cb) => {
  ...
  .then(() => {
    ...
    const globals = {
      ...
      'bootstrap-notify': _.camelCase('bootstrap-notify'.replace('/','.'))
...
gulp.task('serve:demo', () => {
  return execDemoCmd('serve --aot', { cwd: `${config.demoDir}` });
});

gulp.task('serve:demo-hmr', () => {
  return execDemoCmd('serve --configuration hmr --aot', { cwd: `${config.demoDir}` });
});

gulp.task('build:demo', () => {
  return execDemoCmd(`build --preserve-symlinks --prod --base-href /ngx-bootstrap-notify/ --deploy-url /ngx-bootstrap-notify/`, { cwd: `${config.demoDir}`});
});
...
gulp.task('build:demo-ssr', () => {
  return execDemoCmd(`build --preserve-symlinks --prod`, { cwd: `${config.demoDir}`})
    .then(() => execDemoCmd(`run ngx-bootstrap-notify-demo:server`, { cwd: `${config.demoDir}` }))
